...
def get_available_experiments():
    ...
    for entry in os.listdir(EXPERIMENTS_DIR):
        ...
        if os.path.isdir(full_path) and not entry.startswith("."):
            # Check if it contains inference.py to be valid
            if os.path.exists(os.path.join(full_path, "inference.py")):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python inference.py <experiment_name> [experiment args]")
    ...
    sys.argv = [f"./inference.py"] + sys.argv[2:]
    try:
        experiment_module = importlib.import_module(f"{EXPERIMENTS_DIR}.{experiment}.inference")
