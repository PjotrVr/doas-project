...
def get_available_experiments():
    ...
    for entry in os.listdir(EXPERIMENTS_DIR):
        ...
        if os.path.isdir(full_path) and not entry.startswith("."):
            # Check if it contains eval.py to be valid
            if os.path.exists(os.path.join(full_path, "eval.py")):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python eval.py <experiment_name> [experiment args]")
    ...
    sys.argv = [f"./eval.py"] + sys.argv[2:]
    try:
        experiment_module = importlib.import_module(f"{EXPERIMENTS_DIR}.{experiment}.eval")
